...
public class PaymentServiceImpl implements PaymentService {
    ...
    public void assertOrderAccess(Integer orderId, String username, boolean staff) {
        Order order = orderRepository.findById(orderId)
                .orElseThrow(() -> new ResourceNotFoundException("Không tìm thấy đơn hàng: " + orderId));
        if (!staff && !order.getUser().getUsername().equals(username)) {
            throw new BadRequestException("Bạn không có quyền xem hoặc thanh toán đơn hàng này");
        }
    }
    ...
    public PaymentResponse updatePaymentStatus(Integer paymentId, Byte paymentStatus, String transactionId) {
        if (paymentStatus == null || paymentStatus < 0 || paymentStatus > 2) {
            throw new BadRequestException("Trạng thái thanh toán chỉ nhận 0, 1 hoặc 2");
        }
