...
public class ArticleServiceImpl implements ArticleService {
    ...
    private ArticleTopic findOrCreateTopic(String name) {
        ...
        ArticleTopic existing = topicRepository.findByTopicNameIgnoreCase(topicName)
                .or(() -> topicRepository.findBySlug(base))
                .or(() -> findEquivalentTopic(base))
                .orElse(null);
        if (existing != null) return existing;
        return topicRepository.findBySlug(base).orElseGet(() -> {
            String slug = base;
            int suffix = 2;
    ...
    private java.util.Optional<ArticleTopic> findEquivalentTopic(String requestedSlug) {
        return topicRepository.findAll().stream().filter(topic -> {
            String slug = String.valueOf(topic.getSlug());
            if (requestedSlug.contains("bao-quan") || requestedSlug.contains("cham-soc"))
                return slug.contains("bao-quan") || slug.contains("cham-soc");
            if (requestedSlug.contains("phoi-do") || requestedSlug.contains("mix-match"))
                return slug.contains("phoi-do") || slug.contains("mix-match");
            if (requestedSlug.contains("bo-suu-tap") || requestedSlug.contains("lookbook"))
                return slug.contains("bo-suu-tap") || slug.contains("lookbook");
            if (requestedSlug.contains("tin-tuc") || requestedSlug.contains("khuyen-mai"))
                return slug.contains("tin-tuc") || slug.contains("khuyen-mai");
            if (requestedSlug.contains("huong-dan") || requestedSlug.contains("mua-sam"))
                return slug.contains("huong-dan") || slug.contains("mua-sam");
            return requestedSlug.contains("xu-huong") && slug.contains("xu-huong");
        }).findFirst();
    }
