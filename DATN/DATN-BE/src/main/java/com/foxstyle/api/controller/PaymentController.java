...
public class PaymentController {
    ...
    public ResponseEntity<ApiResponse<List<PaymentResponse>>> getPaymentsByOrder(
            @PathVariable Integer orderId, Principal principal,
            org.springframework.security.core.Authentication authentication) {
        assertOrderAccess(orderId, principal, authentication);
    ...
    public ResponseEntity<ApiResponse<PayOSLinkResponse>> checkPayOSStatus(
            @PathVariable Long orderCode, Principal principal,
            org.springframework.security.core.Authentication authentication) {
        assertOrderAccess(orderCode.intValue(), principal, authentication);
    ...
    private void assertOrderAccess(Integer orderId, Principal principal,
                                   org.springframework.security.core.Authentication authentication) {
        boolean staff = authentication.getAuthorities().stream().anyMatch(authority ->
                authority.getAuthority().equals("ROLE_ADMIN") || authority.getAuthority().equals("ROLE_STAFF"));
        paymentService.assertOrderAccess(orderId, principal.getName(), staff);
    }
}
